...
contract Domains is ERC721 {
    ...
    uint256 public basePrice = 1000000000000000000; //100
   ...
   constructor(string memory _tld) payable ERC721("CoinEx Name Service", "CENS") {
    ...
    function withdraw() public onlyOwner {
        ...
        require(success, "Failed to withdraw CET");
    } 
    ...
    function price(string calldata name) public pure returns(uint) {
        ...
        require(len > 0);
        if (len == 3) {
          return 10000 * 10**16; // based on 10000 // * 10**18
        } else if (len == 4) {
	        return 1000 * 10**16; // * 10**18
        } else {
	        return 100 * 10**16; // * 10**18
        }
    }
	...
	function register(string calldata name) public payable {
        ...
        require(msg.value >= _price, "Not enough CET paid");
        ...
        string memory json = Base64.encode(
            bytes(
                ...
                abi.encodePacked(
                    '{"name": "',
                    _name,
                    '", "description": "A domain on the CoinEx names", "image": "',
                    avatar,
                    '","length":"',
                    strLen,
                    '"}'
