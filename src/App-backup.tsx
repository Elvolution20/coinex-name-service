...
const App = () => {
  ...
  useEffect(() => {
    //@ts-ignore
    //if (networks[chainId?.toString(16)] === 'CoinEx Testnet') {
    //if( networks[chainId?.toString(16)]?.includes('COINEX') ){
      if (chainId == 53) {
      fetchMints();
      }
    //}
  ...
  const switchNetwork = async () => {
    if (account) {
      try {
        // Try to switch to the CoinEx Testnet
        await library.send(
          'wallet_switchEthereumChain',
          [{ chainId: '53' }] // Check networks.js for hexadecimal network ids
        );
      } catch (error: any) {
  ...
  const updateDomain = async () => {
    ...
    try {
      if (active) {
        ...
        await tx.wait();
        console.log('Record set https://testnet.coinex.net/#/' + tx.hash);
  ...
  const mintDomain = async () => {
    ...
    try {
      if (active) {
        ...
        if (receipt.status === 1) {
          console.log('Domain minted! https://testnet.coinex.net/#/' + tx.hash);
    ...
    if (chainId !== 53) {
      return (
        <div className="connect-wallet-container">
          ...
          Please switch to CoinEx Testnet
          </button>
        </div>
      );
    }
      ...
      <div className="container">
        ...
        <div className="main-container-wrapper">
          ...
          <>
            <div className="flex-item left">
              <h2>
                Decentralized Name Service <br />
                On CoinEx Blockchain
              </h2>
        ...
        <div className="footer-container">
          <a
            ...
            href="https://twitter.com/coinexnames"
            target="_blank"
            rel="noreferrer"
          >Twitter</a>
        </div>
